package com.moon.sdk.expandablelistview;
...
class SlideExpandableListView extends ListView {
     ...
     * 打开某个item
     * @param position
     * @return @return true if a view was collapsed, false if there was no open view.
     */
    public boolean expandApse(int position){
        if(adapter!=null) {
            return adapter.expandApseItem(position);
        }
        return false ;
    }

    public void setItemExpandCollapseListener(
            AbstractSlideExpandableListAdapter.OnItemExpandCollapseListener listener) {
        if(adapter!=null) {
    ...
    public void removeItemExpandCollapseListener() {
        if(adapter!=null) {
            adapter.removeItemExpandCollapseListener();
        }
    }
