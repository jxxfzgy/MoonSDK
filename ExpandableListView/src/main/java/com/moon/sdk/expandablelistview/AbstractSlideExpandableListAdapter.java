...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
    ...
    /*用于保存记录每个item对应的展开view
    * 以便展开特定的view
    * */
    private Map<Integer, View> viewMap = new ConcurrentHashMap<Integer, View>();
     ...
     * @return
     */
    public boolean expandApseItem(int position) {
        if (!viewMap.containsKey(position)) {
            return false;
        }
        if (position == lastOpenPosition) {
            ...
            collapseLastOpen();
            lastOpen = viewMap.get(position);
            animateView(lastOpen, ExpandCollapseAnimation.EXPAND);
            lastOpenPosition = position;
            notifiyExpandCollapseListener(ExpandCollapseAnimation.EXPAND, lastOpen, lastOpenPosition);
